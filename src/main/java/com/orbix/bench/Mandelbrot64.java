...
public class Mandelbrot64 extends JFrame {
  ...
  private final int[] rgb =
    ((DataBufferInt) image.getRaster().getDataBuffer()).getData(); // extract the underlying RGB buffer from the image
  private final int group = 8; // size of GPU shader groups (default: 32)
  private final Range range = Range.create2D(
    selectGPU(),
    dx - dx % group + (dx % group == 0 ? 0 : group),
    dy - dy % group + (dy % group == 0 ? 0 : group),
    group,
    group
  );
  ...
  public class MandelKernel extends Kernel {
    ...
    @Constant
    private final int MAX = 512; // maximum iterations

    @Constant
    private final int texture[] = new int[MAX]; // texture map
    ...
    public MandelKernel(int dx, int dy, int[] rgb) {
      ...
      for (int i = MAX / 2; i < MAX; i++) {
        texture[i] =
          Color.HSBtoRGB(160f / 360f, 1f, 2f * (1f - (float) i / MAX));
      }
    }
  ...
  private static Device selectGPU() {
    return OpenCLDevice.select(
      new OpenCLDevice.DeviceComparitor() {
        public OpenCLDevice select(
          OpenCLDevice integrated,
          OpenCLDevice dedicated
        ) {
